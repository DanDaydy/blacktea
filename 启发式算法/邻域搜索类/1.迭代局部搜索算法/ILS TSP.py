...
def ILS():
	...
	#计算路径总长度   相当于给出了一个路径，计算整个路径距离，最后加上最后城市返回起点的距离
	def cost_total(city_list):
		cost = 0
		for i in range(CITY_SIZE - 1):
			cost += distance_2city(city_list[i], city_list[i + 1])
		cost += distance_2city(city_list[CITY_SIZE - 1], city_list[0])     #最后一个城市返回起点的距离
		return cost

	#计算两个城市间距离     传入城市1和城市2的下标，得到它们的欧几里得距离
	...
	def local_search(best_solution):               #给一个初始解，开始搜索其邻域

		#颠倒数组中下标begin到end的元素位置       在传入的路径中指定了两个位置，将两个位置中间的城市进行互换
		...
		#邻域动作 反转index_i <-> index_j 间的元素     传入一个路径和指定两个位置，执行上面的互换函数
		def two_opt_swap(cities_route, index_i, index_j):
			new_cities_route = copy.deepcopy(cities_route)    #内置的deepcopy()函数
			swap_element(new_cities_route, index_i, index_j)
			return new_cities_route

		# 计算邻域操作优化值      经过邻域操作之后比之前优化了还是拉跨了呢
		def calc_delta(i, k, city_list):
			'''
                ...
                传入  1,3,1-2-3-4-5   此时i2=0,k2=4   delta = -d[0][1] + d[0][3] -d[3][4] + d[1][4]
                									delta = -d12   +   d14   - d45  + d25
			'''
			delta = 0
			if i == 0 and k == CITY_SIZE - 1 :      #不幸首尾互换的话，总cost并没有变
				delta = 0
			else :           #这里的处理是为了防止出现i ==0 或者  k == CITY_SIZE - 1
		...
		#迭代出Delta矩阵，i行k列代表的意思是  原解交换i,k之间的元素后的变化值delta
		for i in range(CITY_SIZE - 1):
        ...
        # 枚举排列
			...
			for i in range(CITY_SIZE - 1):
				for k in range(i + 1, CITY_SIZE):
					# 邻域动作
					new_solution.route = two_opt_swap(best_solution.route, i, k)
	...
	# 判断接受准则    新的路径花费比原来的高于500以上，返回False，其它返回True
	def AcceptanceCriterion (cities_route, new_cities_route):
		AcceptLimite = 500
		c1 = cost_total(cities_route)
		c2 = cost_total(new_cities_route) - AcceptLimite
		if c1 < c2 :
			return False
		else : 
			return True
	...
	def double_bridge_move(cities_route, new_cities_route):
		...
		while True:
			...
			if AcceptanceCriterion(cities_route, new_cities_route) : break
	...
	random.shuffle(best_solution.route)    #将所有元素随机排序
	...
	for i in range(max_iterations):
		...
		print("迭代搜索 ", i, " 次   ", "最优解 = ", best_solution.cost, " 当前解 = ", current_solution.cost)
	result = dict()
