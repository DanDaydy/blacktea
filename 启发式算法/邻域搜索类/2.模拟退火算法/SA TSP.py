...
def cal_distance(id1,id2):
    distance = math.sqrt((city[id1][0] - city[id2][0])**2 +  (city[id1][1] - city[id2][1])**2)
    return distance
...
def SA():
	...
	best_route = [] # 全局最优解
	for i in range(N):
		best_route.append(i)         #生成初始解
	best_route.append(0)             #最后加上初始点0
	best = path_len(best_route)      #最优路径长度
	T = T0
	while T > T_min :
		for i in range(L):   #实际上这里应该是用L，即设置的在每个温度迭代1000次，原代码使用N似乎是毫无意义的
